...
def show_mask(ax, mask: np.ndarray, random_color=False):
    ...
    h, w = mask.shape[:2]  # Use only the height and width for single and multi-channel masks
    print(mask.size)
    print("h:", h)
    print("w:", w)
    ...
    if mask.ndim == 2:  # Single-channel mask
        mask_img = mask.reshape(h, w, 1) * color[:3].reshape(1, 1, -1)
    elif mask.ndim == 3 and mask.shape[2] == 3:  # Three-channel mask
        mask_img = mask * color[:3].reshape(1, 1, -1)
